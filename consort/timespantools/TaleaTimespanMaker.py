...
class TaleaTimespanMaker(TimespanMaker):
    ...
    ::
        ...
        consort.timespantools.TaleaTimespanMaker(
            can_split=True,
            initial_silence_talea=rhythmmakertools.Talea(
                counts=(0, 4),
                denominator=16,
    ...
    def _fuse_timespans(self, timespans):
        ...
        if len(timespans) == 1:
            ...
            pass
        elif isinstance(timespans[-1], consort.timespantools.PerformedTimespan):
            fused_timespan = new(
                timespans[1],
                stop_offset=timespans[-1].stop_offset,
                )
        ...
        while start_offset < stop_offset and can_continue:
            ...
            for voice_name, music_specifier in music_specifiers.items():
                ...
                for i, duration in enumerate(durations):
                    ...
                    if self.padding:
                        if i == 0:
                            timespan = consort.timespantools.SilentTimespan(
                                layer=layer,
